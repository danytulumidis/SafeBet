...
const CryptoBet = () => {
  ...
  const [timeLeftForBet, setTimeLeftForBet] = useState(0)
  ...
  const placeBetDown = async (amount) => {
    try {
      ...
      const tx = await contract.placeBetDown({ value: utils.parseEther(amount) });
  ...
  const placeBetUp = async (amount) => {
    try {
      ...
      const tx = await contract.placeBetUp({ value: utils.parseEther(amount) });
  ...
  const getTimesLeft = async () => {
    ...
    /* TimeStamp handler */
    const txBet = await betContract.getTimeLeft();
    setTimeLeftForBet(BigNumber.from(txBet).toNumber())
            ...
            <div className='overflow-auto h-screen pb-24 pt-2 pr-2 pl-2 md:pt-0 md:pr-0 md:pl-0'>
              <div className='flex flex-col flex-wrap sm:flex-row '>
                ...
                <div className='w-full sm:w-1/2 xl:w-1/3'>
                  ...
                  <div className='mb-4'>
                    <div className=' shadow-lg rounded-2xl p-4 bg-white dark:bg-gray-900 w-full'>
                      ...
                      <div className='mt-8 flex justify-center items-center w-full'>
                        <button onClick={() => placeBetUp("1.1")} className='border-2 border-secondary-color p-4 rounded-full mx-8 transition-all duration-500 ease-in-out hover:scale-90'>
                          <svg
                            width='30px'
                            height='30px'
                            viewBox='0 0 30 30'
                        ...
                        <div ref={divRef} contentEditable={true} suppressContentEditableWarning={true} className='text-white text-4xl font-bold'>
                          1.1 MATIC
                          {/* 
                          TODO: Add ETH Price 
                          */}
                        </div>
                        <button onClick={() => placeBetDown("1.1")} className='border-2 border-secondary-color p-4 rounded-full mx-8 transition-all duration-500 ease-in-out hover:scale-90'>
                          <svg
                            width='30px'
                            height='30px'
                            viewBox='0 0 30 30'
                  ...
                  <div className='mb-4'>
                    <div className='shadow-lg rounded-2xl p-4 bg-white dark:bg-gray-900 w-full'>
                      <h1 className='text-4xl mb-4 font-bold text-white text-center w-full'>
                        Time Left
                      </h1>
                      <div className='mt-8 flex justify-center items-center w-full'>
                        <h2 className=' text-6xl font-bold text-transparent bg-clip-text bg-gradient-to-b from-pink-700 to-sf-violet'>
                          {Math.floor(timeLeftForBet / 3600) % 24}:{Math.floor(timeLeftForBet / 60) % 60}:{Math.floor(timeLeftForBet % 60)}
                          {/* 
                          TODO: Add Time Left 
                          */}
                        </h2>
                ...
                <div className='w-full sm:w-1/2 xl:w-1/3'>
                  ...
                  <div className='mb-4'>
                    <div className='shadow-lg rounded-2xl bg-white dark:bg-gray-900 w-full'>
                      ...
                      <p className='font-bebas text-4xl tracking-wider p-4 text-black dark:text-white'>
                        Previous Price <em className="text-sm">(3 seconds before)</em>
                      </p>
                      <p className='text-2xl p-4 text-black dark:text-white'>
                        ${prevETHPrice}
                        {/*
